...
This experiment asked whether a model could make that more specific diagnosis from only the first six conversation turns. I used six turns as a fixed opening window, intended as a rough proxy for the first 60–90 seconds. I did not establish that this is the right cutoff for real calls.
...
2. Another model read only the conversation text and predicted the cause; it could not see the answer key.
3. A separate judging step compared its prediction with the intended answer. GPT created and judged the conversations; Claude predicted the cause.
...
That is the direction of [Copilot Lab](https://github.com/gititya/support-copilot-lab): private assistance for the human rep during an investigation. It should keep track of facts, competing explanations and the evidence needed before recommending a fix or a transfer. The rep remains in control of what to ask and whether the proposed fix worked.
